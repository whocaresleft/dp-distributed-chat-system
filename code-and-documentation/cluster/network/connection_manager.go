...
// Connection manager is the node's component that handles communication with other nodes in the system.
// It handles election messages, heartbeat (post election) messages and request forwarding
type ConnectionManager interface {
	Bind(port uint16) error                 // Binds the connection with the given port
	GetIdentity() string                    // GetIdentity Returns the identity of the socket
	ConnectTo(address string) error         // ConnectTo Connects to address, enstablishing a connection
	DisconnectFrom(address string) error    // DisconnectFrom Closes the connection with address
	SwitchAddress(old, new string) error    // SwitchAddress Replaces the connection with old with one with new.
	SendTo(id string, payload []byte) error // SendTo sends payload to the connected peer, identified by id.
	Recv() ([][]byte, error)                // Recv receives a [][]byte message on the socket, and returns it
	Poll(timeout time.Duration) error       // Poll waits for an amount of time, timeout, for an event to occur.
	Destroy()                               // Destroy closes the current socket
}

// ZeroMQConnectionManager is an implementation of connection manager using zeromqsockets
type ZeroMQConnectionManager struct {
	ctx    *zmq.Context
	socket *zmq.Socket
	poller *zmq.Poller
}
...
func NewZeroMQConnectionManager(id node.NodeId) (*ZeroMQConnectionManager, error) {
	...
	return &ZeroMQConnectionManager{
		context,
		r,
		p,
	}, nil
...
func (c *ZeroMQConnectionManager) StartMonitoring(handshakeCallback func(string)) {
...
func (c *ZeroMQConnectionManager) Bind(port uint16) error {
...
func (c *ZeroMQConnectionManager) GetIdentity() string {
	i, _ := c.socket.GetIdentity()
	return i
}

// ConnectTo Connects to address, enstablishing a connection
func (c *ZeroMQConnectionManager) ConnectTo(address string) error {
...
func (c *ZeroMQConnectionManager) DisconnectFrom(address string) error {
	fullAddr := getFullAddress(address)
...
func (c *ZeroMQConnectionManager) SwitchAddress(old, new string) error {
...
// When successful error is nil
func (c *ZeroMQConnectionManager) SendTo(id string, payload []byte) error {
...
func (c *ZeroMQConnectionManager) Recv() ([][]byte, error) {
...
func (c *ZeroMQConnectionManager) Poll(timeout time.Duration) error {
...
func (c *ZeroMQConnectionManager) Destroy() {
	c.socket.Close()
	c.ctx.Term()
}
